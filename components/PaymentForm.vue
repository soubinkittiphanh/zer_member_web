    ...
    methods: {
        async commitRecord() {
            if (this.$refs.form.validate() && !this.isloading) {
                ...
                this.isloading = true
                let api = this.isCreate ? 'api/paymentMethod/create' : `api/paymentMethod/update/${this.recordId}`
                console.log("API => ", api);
                if (this.isCreate) {
        ...
        async loadEntry() {
            ...
            if (this.recordId && !this.isCreate) {
                await this.$axios.get(`api/paymentMethod/find/${this.recordId}`).then(response => {

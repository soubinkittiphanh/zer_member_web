<template>
    <div>
            ...
            <v-card-text class="ma-0">
                <v-row>
                    ...
                    <v-col class="pa-0" cols="2">
                        <v-text-field type="number" outlined v-model="amount" label="* ຈຳນວນ ລ່າງ" @input="amountTypingTrigger"
                            @focus="amountTypingTrigger"></v-text-field>
                    </v-col>
                    <v-col class="pa-0" cols="3">
                        <v-text-field type="number" outlined v-model="amountUp" label="* ຈຳນວນ ບົນ" @input="amountUpTypingTrigger"
                            @focus="amountUpTypingTrigger"></v-text-field>
                    </v-col>
                    <v-col cols="4">
        ...
        <v-card style="  height: 20vh; position: fixed; left: 0; right: 0; z-index: 2; margin-top: 50vh; ">
            ...
            <v-card-text>
                <v-row>
                    ...
                    <v-col cols="4">
                        <v-btn color="primary" variant="text" @click="addTransaction">
                            <h2>
                                ລວມບິນ
                            </h2>
                        </v-btn>
        ...
        <!-- Amount option -->
        <v-card v-if="amountOption" v-scroll-x="onscroll" @scroll="onScrollAmount" class="text-center transparent"
        ...
        <!-- Amount option up -->
        <v-card v-if="amountOptionUp" v-scroll-x="onscroll" @scroll="onScrollAmountUp" class="text-center transparent"
            style="width:100%; overflow-x: auto;position: fixed; z-index: 2; bottom: 60vh;left: 0;right: 0;">
            <v-btn v-for="( op, i ) in  amountOptionForSelect " :key="i" color="primary" width="20" rounded variant="text"
    ...
    methods: {
        ...
        selectLuckyNumber() {
            for (const iterator of this.luckyNumberOptionForSelect) {
                ...
                const existTxn = this.transactionList.find(el => el['luckyNumber'] == iterator)
                if (existTxn != undefined) {
                    existTxn['amount'] += this.amount
                    continue
                }
                this.transactionList.push(
                    { luckyNumber: iterator, amount: this.amount > 1000 ? this.amount : 1000 },
                )
            }
            this.luckyNumberOption = false
        },
        ...
        addTransaction() {
            ...
            const existTxn = this.transactionList.find(el => el['luckyNumber'] == this.luckyNumber)
            if (existTxn != undefined) {
                return existTxn['amount'] += this.amount
            }
            this.transactionList.push(
                { luckyNumber: this.luckyNumber, amount: this.amount },
            )
        },
        removeTransaction(element) {
            const indexInOddList = this.transactionList.findIndex((item) => item.luckyNumber === element['luckyNumber']);
